...
namespace RangeSlider
{
    public class RangeSlider : Control
    {
        ...
        /// <summary>
        /// The maximum range span dependency property.
        /// </summary>
        public static readonly DependencyProperty MaximumRangeSpanProperty =
            DependencyProperty.Register("MaximumRangeSpan", typeof(double), typeof(RangeSlider), new PropertyMetadata(0.0));
        ...
        public double Minimum
        {
            ...
            set
            {
                ...
                double NewSpan = this.Maximum - this.Minimum;
                if (NewSpan < this.MinimumRangeSpan)
                {
                    this.MinimumRangeSpan = NewSpan;
                }

                /* if (NewSpan > this.MaximumRangeSpan)
                {
                    this.MaximumRangeSpan = NewSpan;
                } */
            }
        }
        ...
        /// <summary>
        /// Gets or sets the slider maximum range span.
        /// </summary>
        public double MaximumRangeSpan
        {
            get
            {
                return (double)GetValue(MaximumRangeSpanProperty);
            }

            set
            {
                SetValue(MaximumRangeSpanProperty, Math.Max(this.Maximum - this.Minimum, value));
                this.UpdateSelectedRangeMinimumWidth();
                this.UpdateRange(false);
            }
        }
        ...
        private void UpdateRange(bool raiseEvent)
        {
            if (this.selectedRangeBorder != null)
            {
                ...
                else if (rangeEnd - rangeStart > this.MaximumRangeSpan)
                {
                    /*if (rangeStart + this.MaximumRangeSpan > this.Maximum)
                    {
                        rangeStart = this.Maximum - this.MaximumRangeSpan;
                    }*/

                    if (rangeStart < this.RangeStart)
                    {
                        // the left slider was moved
                        // Let it move freely and drag the other slider
                        rangeEnd = rangeStart + this.MaximumRangeSpan;
                    }
                    else
                    {
                        // The right slide was moved
                        // Let it move freely and drag the other slider
                        rangeStart = rangeEnd - this.MaximumRangeSpan;
                    }

                    //rangeEnd = Math.Min(this.Maximum, rangeStart + this.MaximumRangeSpan);
                    rangeChanged = true;
                }
